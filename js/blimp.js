			...
			var blimpMouseX = 0, blimpMouseY = 0;
			...
			function initBlimp() {
				...
				cameraBlimp = new THREE.PerspectiveCamera( 45, 500 / window.innerHeight, 1, 2000 );
				...
				rendererBlimp.setSize( 500, window.innerHeight );

				rendererBlimp.setClearColor( 0xffffff, 0 );
				blimpContainer.appendChild( rendererBlimp.domElement );

				document.addEventListener( 'mousemove', onBlimpMouseMove, false );
			...
			function onWindowResize() {
				...
				cameraBlimp.aspect = 500 / window.innerHeight;
			...
			function onBlimpMouseMove( event ) {
				blimpMouseX = ( event.clientX - windowHalfX ) / 50;
				blimpMouseY = ( event.clientY - windowHalfY ) / 50;
			}
			...
			function renderBlimp() {
				//console.log('render blimpy')
				var timer = 0.001 * Date.now();
				cameraBlimp.position.x += ( blimpMouseX - cameraBlimp.position.x ) * .1;
